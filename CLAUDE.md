...
**Every `QDialog` must call `.setStyleSheet(Style.DialogStyle)`.** Qt gives an unstyled `QDialog` the OS's native/default look, not a transparent pass-through — on Windows that frequently renders dark even when the app's own theme is Light, since it follows the system's own dark/light setting rather than `Style.Mode`. `BuildCompletedWindow`'s `Dialog` and `EditStartMenuPath`'s `self.PathDialog` were missed when they were added (their child widgets were themed individually, but never the dialog background itself), which is exactly what made those two windows look "stuck in dark mode" regardless of the selected theme. When adding a new `QDialog`, style it immediately after construction, the same way every other dialog in this file does.
...
Both wizards include a mandatory **License Agreement** step before install proceeds, whose text (`AGREEMENT_TEXT` in `installer_app.py`, `Installer/LICENSE_AGREEMENT.txt` for Inno Setup — kept in sync manually) discloses the local `.py`/`.ico` file indexing described below under Local/runtime config: it explains what's scanned, that paths/names (never file contents) are stored only in `user-data/db/py_index.db` and `icon_index.db` on the user's own machine, and that nothing is sent over the network. The PySide6 page gates its Next button on a checkbox (`AgreementPage.isComplete`); the Inno Setup page uses the built-in `LicenseFile` directive's Accept/Decline radios. If the indexing behavior ever changes (what's scanned, where it's stored, whether it becomes non-local), update both texts together.
